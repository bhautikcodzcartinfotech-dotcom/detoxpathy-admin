...
const SettingsPage = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const finalData = {
        ...
        newPatientOnboardStop: typeof settingsData.newPatientOnboardStop !== 'undefined' ? settingsData.newPatientOnboardStop : false,
        ...
        version: typeof settingsData.version !== 'undefined' ? String(settingsData.version) : '',
        iosVersion: typeof settingsData.iosVersion !== 'undefined' ? String(settingsData.iosVersion) : '',
        ...
        supportPannelSound: settingsData.supportPannelSound || "/1783405039988mixkit-bell-notification-933.wav",
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (formData.bannerFile || formData.audioFile || formData.appointmentSoundFile || formData.supportPannelSoundFile) {
        ...
        if (formData.supportPannelSoundFile) data.append('supportPannelSound', formData.supportPannelSoundFile);
  ...
  const handleSupportPannelSoundUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    ...
    reader.onloadend = () => {
      setFormData(prev => ({
        ...prev,
        supportPannelSound: reader.result,
        supportPannelSoundFile: file
      }));
    };
    reader.readAsDataURL(file);
    toast.success("Support panel sound selected! Click 'Update Settings' to save.");
  };
        ...
        <form className="space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gradient-to-br from-white to-amber-50 rounded-2xl shadow-lg border border-amber-200 p-6">
              ...
              <div
                className={`px-4 py-2 rounded-full text-sm font-medium inline-block ${(formData.screenshotProtectionActive ?? true)
                  ? "bg-amber-200 text-yellow-900"
                  : "bg-red-100 text-red-800"
                  }`}
              >
            ...
            {/* New patient Onboard stop */}
            <div className="bg-gradient-to-br from-white to-amber-50 rounded-2xl shadow-lg border border-amber-200 p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-2">
                    New Patient Onboarding Stop
                  </h3>
                  <p className="text-sm text-gray-600">
                    Stop self-registration/onboarding of new patients when active
                  </p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    checked={formData.newPatientOnboardStop || false}
                    onChange={(e) =>
                      handleInputChange("newPatientOnboardStop", e.target.checked)
                    }
                    className="sr-only peer"
                  />
                  <div className="w-14 h-7 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-yellow-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-yellow-500"></div>
              ...
              <div
                className={`px-4 py-2 rounded-full text-sm font-medium inline-block ${formData.newPatientOnboardStop
                  ? "bg-red-100 text-red-800"
                  : "bg-green-100 text-green-800"
                  }`}
              >
                {formData.newPatientOnboardStop ? "Registration Stopped" : "Registration Allowed"}
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-white to-amber-50 rounded-2xl shadow-lg border border-amber-200 p-6">
              ...
              <input
                ...
                onChange={(e) => {
                  handleInputChange("iosVersion", e.target.value);
                }}
                className="w-full px-4 py-3 rounded-xl border border-amber-200 focus:ring-2 focus:ring-yellow-400 focus:border-transparent bg-white/50 transition-all duration-200 outline-none font-semibold text-gray-700"
                placeholder="1.0.0"
              />
          ...
          <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 transition-all hover:shadow-2xl">
            <div className="flex items-center gap-4 mb-8">
              <div className="w-12 h-12 rounded-2xl bg-violet-100 flex items-center justify-center text-violet-600 shadow-inner">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h6m-6 4h10m4-8v10a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h14a2 2 0 012 2z" />
                </svg>
              </div>
              <div>
                <h3 className="text-xl font-bold text-gray-900">Support Panel Sound</h3>
                <p className="text-sm text-gray-500">Upload a sound to use for support panel notifications</p>
              </div>
            </div>

            <div className="space-y-6">
              {formData.supportPannelSound ? (
                <div className="p-6 rounded-2xl bg-violet-50/50 border border-violet-200/60 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-violet-800 uppercase tracking-widest">Active Support Panel Sound</span>
                    <button
                      type="button"
                      onClick={() => setFormData(prev => ({ ...prev, supportPannelSound: "", supportPannelSoundFile: null }))}
                      className="text-xs text-red-500 hover:text-red-600 font-bold hover:underline"
                    >
                      Remove Sound
                    </button>
                  ...
                  <audio
                    src={formData.supportPannelSound.startsWith('data:') ? formData.supportPannelSound : getVideoUrl(formData.supportPannelSound)}
                    controls
                    className="w-full"
                  />
                </div>
                ...
                <div className="relative group w-full">
                  <label htmlFor="support-pannel-sound-upload" className="flex flex-col items-center justify-center w-full min-h-[140px] cursor-pointer p-6 border-4 border-dashed border-gray-200 rounded-[2rem] bg-gray-50 hover:bg-violet-50/30 hover:border-violet-300 transition-all duration-300 shadow-inner">
                    <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-md mb-3 text-gray-300 group-hover:text-violet-400 transition-colors">
                      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                      </svg>
                    </div>
                    <p className="text-sm font-semibold text-gray-500 group-hover:text-violet-600 transition-colors">Select an audio file</p>
                    <p className="text-[10px] text-gray-400 mt-1">Accepts MP3, WAV or AAC (Max 5MB)</p>
                  </label>
                </div>
              )}
              ...
              <div className="flex justify-center">
                <input
                  type="file"
                  accept="audio/*"
                  onChange={handleSupportPannelSoundUpload}
                  className="hidden"
                  id="support-pannel-sound-upload"
                />
                <label
                  htmlFor="support-pannel-sound-upload"
                  className="group px-8 py-4 bg-gradient-to-r from-gray-900 to-gray-800 text-white font-bold rounded-2xl hover:from-violet-600 hover:to-violet-500 transition-all duration-300 cursor-pointer flex items-center gap-3 shadow-xl hover:shadow-violet-200 hover:-translate-y-0.5"
                >
                  <svg className="w-5 h-5 transition-transform group-hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                  </svg>
                  {formData.supportPannelSound ? "Replace Support Panel Sound" : "Upload Support Panel Sound"}
                </label>
              </div>
            </div>
          </div>
