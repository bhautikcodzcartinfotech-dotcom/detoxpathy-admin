...
import { listCoupons, createCoupon, updateCoupon, deleteCoupon } from "@/Api/AllApi";
import CouponForm from "./CouponForm";
import CouponTable from "./CouponTable";

const PromoVaultPage = () => {
  ...
  const handleSubmit = async (formData) => {
    try {
      ...
      setError("");
      if (editing) {
        await updateCoupon(editing._id, formData);
    ...
    <RoleGuard allow={["Admin"]}>
      <div className="w-full h-full px-6 py-4">
        <div className="flex items-center justify-between mb-4 px-8">
          <Header size="3xl">Coupon Code</Header>
          <Button
            onClick={() => {
              setEditing(null);
              setIsOpen(true);
