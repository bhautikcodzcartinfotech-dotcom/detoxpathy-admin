...
const AppointmentPage = () => {
  ...
  const [viewMode, setViewMode] = useState("daily"); // "daily" or "tomorrow"
  ...
  const toggleConsultationPanel = async () => {
    if (!showConsultationPanel) {
      if (activeCallAppointment?.userId?._id && !userOverviewData) {
        try {
    ...
    <RoleGuard allow={["Admin", "subadmin"]} permission="show appointments page">
      <div className="w-full h-full px-4 sm:px-6 lg:px-8 bg-[#F8FAFC] min-h-screen pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { label: viewMode === "daily" ? "TODAY TOTAL" : "TOMORROW TOTAL", value: stats.total, color: "border-teal-600", icon: LayoutGrid, iconColor: "text-teal-600" },
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl shadow-inner flex-1 sm:flex-none justify-center h-12">
              ...
              <button
                ...
                className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all h-full ${viewMode === "tomorrow" ? 'bg-white text-teal-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                Tomorrow
              </button>
            </div>
                  ...
                  {filteredAppointments.map((item) => {
                    ...
                    return (
                      <tr key={item._id} className="group hover:bg-gray-50/80 transition-all duration-200">
                        <td className="px-6 py-6 text-[14px] font-bold text-gray-800">
                          <div>
                            <p>{item.userId?.name} {item.userId?.surname}</p>
                            {item.isNewuser && (
                              <p className="mt-1 text-[10px] font-black uppercase tracking-widest text-teal-600">New user</p>
                            )}
                            {(item.planCurrentDay || item.nextFollowUpPlanDate) && (
                              <p className="mt-1 text-[11px] font-medium text-violet-600">
                                {item.planCurrentDay && `Detox Day: ${item.planCurrentDay}`}
                                {item.planCurrentDay && item.nextFollowUpPlanDate && " · "}
                                {item.nextFollowUpPlanDate && `Next follow-up: ${item.nextFollowUpPlanDate}`}
                              </p>
                            )}
                          </div>
                        </td>
                  ...
                  <div key={item._id} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm transition-all active:scale-[0.98]">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex flex-col">
                        ...
                        {(item.planCurrentDay || item.nextFollowUpPlanDate) && (
                          <p className="mt-1 text-[11px] font-medium text-violet-600">
                            {item.planCurrentDay && `Plan day: ${item.planCurrentDay}`}
                            {item.planCurrentDay && item.nextFollowUpPlanDate && " · "}
                            {item.nextFollowUpPlanDate && `Next follow-up: ${item.nextFollowUpPlanDate}`}
                          </p>
                        )}
        ...
        {/* Offline Appointment Modal: Profile + Consultation Form (single scroll) */}
        {activeOfflineAppointment && (
          <div className="fixed inset-0 z-50 flex bg-slate-950/30 backdrop-blur-sm">
            <motion.div
              ...
              exit={{ opacity: 0, y: 30 }}
              className="relative w-full h-full flex flex-col overflow-hidden bg-white shadow-2xl"
            >
                ...
                {offlineUserOverview ? (
                  <div className="p-4 sm:p-8 space-y-6">
                    <ConsultationForm
                      ...
                      patient={offlineUserOverview?.user}
            ...
            >
              ...
              <AnimatePresence>
                {showConsultationPanel && (
                  ...
                  >
                    <div className="flex items-center justify-between sticky top-0 bg-white/95 backdrop-blur-md z-30 py-4 px-4 sm:px-8 border-b border-slate-100 shrink-0">
                      <h4 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Consultation</h4>
                      <button
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-row items-center gap-2 sm:gap-4 flex-wrap">
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 rounded-full px-3 sm:px-6 py-1.5 sm:py-2.5 text-[9px] sm:text-xs font-black uppercase tracking-widest transition-all ${showConsultationPanel ? 'bg-teal-600 text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 shadow-sm'}`}
                      >
                        {loadingUserOverview ? (
                          <div className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" />
                        ) : (
                        ...
                        <span>{showConsultationPanel ? "Hide Consultation" : "Consultation"}</span>
                      </button>
                    </div>
                  </div>
                </div>
